                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {parseGallery(p.gallery).map((src, i) => (
                        <img key={i} src={typeof src === 'object' && src?.base64 ? src.base64 : src} alt="" style={{ width: 64, height: 64, borderRadius: 8, objectFit: 'cover', border: '1px solid var(--border)' }} />
                      ))}
                    </div>
                  </div>
                )}
                    ...
                    <ul style={{ margin: 0, paddingLeft: 20 }}>
                      {(toursByProviderId[p.id] || []).map((t) => (
                        <li key={t.id} style={{ marginBottom: 4 }}>
                          <Link to={`/app/tour/${t.id}`} style={{ color: 'var(--gold)', fontSize: '0.9rem' }}>{t.name}</Link>
